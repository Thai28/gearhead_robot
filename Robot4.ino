...
// Servo PWM value
# define FLAT_ANGLE 410 // Góc 180 độ
# define RIGHT_ANGLE 205; // Góc 90 độ
// State variables
...
void triangleButtonHandler()
{
  if (ps2x.Button(PSB_GREEN)) // Nút xanh lá (nút tam giác) mở / đóng cổng bắn bóng
  {
    ...
    pwm.setPWM(5, 0, ballLaunchGateState ? FLAT_ANGLE : 0); // Mở cổng nếu đang đóng và đóng cổng nếu đang mở
  }
}
void squareButtonHandler()
{
  if (ps2x.Button(PSB_PINK)) // Nút hồng (nút vuông) mở / đóng cổng thả bóng
  {
    ...
    int pwmValue = ballDropGateState ? FLAT_ANGLE : 0;
...
void ballSortingHandler()
{
  ...
  {
    pwm.setPWM(SORTER_SERVO_PIN, 0, RIGHT_ANGLE); // Mở servo tại góc 90 độ
  }
  else 
  {
    pwm.setPWM(SORTER_SERVO_PIN, 0, 0); // Đóng servo
